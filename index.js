...
function createBot() {
  ...
  // ---- Bot Dancing ----
  function makeBotDance(bot) {
    console.log("[Dance] Bot started dancing!");
    ...
    const danceInterval = setInterval(() => {
      ...
      // Spin randomly (sometimes do a full 360 spin!)
      if (Math.random() < 0.3) {
        bot.look(bot.entity.yaw + 2 * Math.PI, 0, true); // full spin
      } else {
        bot.look(bot.entity.yaw + Math.PI / 2, 0, true); // quarter spin
      }

      // Random chat emotes
      const emotes = [""];
      if (Math.random() < 0.2) {
        bot.chat(emotes[Math.floor(Math.random() * emotes.length)]);
      }
    }, 300);

    // Loop forever (remove stop timer)
  }
  ...
  bot.on('goal_reached', () => {
    console.log(
      `\x1b[32m[AfkBot] Bot arrived at target location. ${bot.entity.position}\x1b[0m`
    );
  });
